...
**Source of Truth:** Actual Git history + actual files on `main` at `02a065070ba88c75f016956ae651a3269c55da63`
...
| Ref                                         | SHA                                        | Status                            |
| ------------------------------------------- | ------------------------------------------ | --------------------------------- |
| **HEAD (main)**                             | `02a065070ba88c75f016956ae651a3269c55da63` | ✅ Clean, synced with origin/main |
| **origin/main**                             | `02a065070ba88c75f016956ae651a3269c55da63` | ✅ Synced                         |
...
| ----------------------------------------- | ------- | ----------------------- | ---------- | ----------------------- | ------------------- | -------------------------- |
| `main`                                    | Shivam  | Release baseline        | `02a0650`  | HEAD                    | Released            | —                          |
...
| G15 — ML final candidate         | **PARTIAL**           | M17 research-locked (SHA256 `D7C0BE91…EDAC`); canonical backend candidate implemented on this branch (`M17DepthBackend`, fake-tested); formal test-city verification pending | Shravan      | **Yes**            |
| G15A — Solar-shadow sub-gate     | **PASS** (capability) | `ShadowObservation` → `ShadowHeightConstraint` implemented; `tests/solar/` pass; not PS-required per portal                                                                  | Shivam       | No                 |
| G15B — Neural-rendering sub-gate | **PASS** (decision)   | Raster baseline reaffirmed; neural recorded as explicit optional future; not PS-required per portal                                                                          | Aryan/Shivam | No                 |
...
- `docs/final-release-gate.md` — Canonical G1–G19 matrix (updated this branch with G15A/G15B)

---

## 15. Final Release Decision

### **RELEASE CANDIDATE — PHYSICAL WITNESS REQUIRED** (with scientific-candidate requirement noted; PS solar/neural resolved: solar implemented as cue, neural decided)
...
**End of Final Release Status.** This document reflects the actual state of `main` at `02a065070ba88c75f016956ae651a3269c55da63` as verified by Git commands, file inspection, test execution, and actual build/artifact generation.
