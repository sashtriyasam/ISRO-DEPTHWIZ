...
**Repository State:** `main` at commit `c08406a`  
...
1. **Engineering Validation $\neq$ Scientific Accuracy**: 100% test suite completion, clean TypeScript compilation, and executable packaging prove system software engineering, **not** universal metric accuracy across arbitrary global terrain.
2. **Relative Depth $\neq$ Metric DSM**: Non-georeferenced optical RGB inputs (PNG/JPG) produce **Relative Digital Surface Models (`rDSM`)** in local coordinate frames (`units=None`, `is_metric=False`). Metric claims require explicit calibration.
...
| **Evidence** | - 549 Python pytest tests passed<br>- 627 Vitest UI tests passed<br>- 0 TypeScript compilation errors<br>- 0 Ruff linter issues<br>- 86 Mypy clean source files<br>- Authenticode signed installer (`2A974B51...`) <br>- 20/20 Physical Witness Trial items passed | - 32-tile GAMUS pooled evaluation (MAE 4.40m, RMSE 5.86m, $R^2$ 0.23)<br>- Cross-city probe Pearson correlation: 0.37 (6/6 cities)<br>- Formal external test-city scoring: **`PENDING`** |
...
The ML research track (Shravan) conducted cross-city structural adaptation probes across 6 diverse urban landscapes (GeoNRW probe):

| Model Candidate | Probe Pearson Correlation (6/6 Cities) | Status |
| :--- | :--- | :--- |
| **M10 Baseline** | $0.25$ | Historical ML milestone |
| **M17 Structural Head** | **$0.37$** | **Frozen Research Candidate** (`best.pt`, SHA-256 `D7C0BE91...`) |
...
- **Signed Installer SHA-256**: `2A974B514694D79C0B7E72D6F17EE33B2B07A532CDD33207F9D34FFB3452D717`

### Environment Isolation
- Provisioned via [`scripts/provision_runtime.py`](file:///d:/SIH%20DEPH%20WIZARD/scripts/provision_runtime.py) into isolated virtual environment.
- Verified via [`scripts/runtime_check.py`](file:///d:/SIH%20DEPH%20WIZARD/scripts/runtime_check.py) with zero top-level heavy imports during core initialization.
...
3. **Solar Angle & Shadow Occlusion**: Deep cast shadows in high-relief mountainous or urban canyon regions degrade relative depth estimation accuracy.
