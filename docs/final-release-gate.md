...
| **G15** SIH problem-statement compliance | **PARTIAL**           | Per `docs/final-sih-compliance.md` (16 items): solar/neural NOT REQUIRED by official PS (portal-verified); single-tile scope PARTIAL; remainder PASS/contract                                                                                                                                                                                          | Shivam         | **Yes** (scope)    |
| **G15A** Solar-shadow sub-gate           | **PASS** (capability) | `ShadowObservation` → `ShadowHeightConstraint` implemented (`src/depthwizard/solar/`), explicit trig, honest refusal; `tests/solar/` pass (14); not PS-required per portal                                                                                                                                                                             | Shivam         | No                 |
...
| SIH compliance gaps (solar, neural rendering) | G15/G15A/G15B    | RESOLVED: solar implemented as independent cue; neural decided (baseline + optional future); neither PS-required per portal |
...
| Decision                                              | Condition                                                           |
| ----------------------------------------------------- | ------------------------------------------------------------------- |
| **NOT RELEASE READY**                                 | Physical Windows acceptance NOT VERIFIED; ML candidate not promoted |
| **RELEASE CANDIDATE — PHYSICAL WITNESS REQUIRED**     | All automated gates PASS; physical witness required                 |
| **RELEASE CANDIDATE — SCIENTIFIC CANDIDATE REQUIRED** | ML freeze/M17 promotion required                                    |
| **RELEASE CANDIDATE**                                 | —                                                                   |
| **READY FOR SHIVAM RELEASE AUTHORIZATION**            | All required gates have real evidence (not current state)           |

**Current State:** **RELEASE CANDIDATE — PHYSICAL WITNESS REQUIRED** (with scientific-candidate requirement noted; PS solar/neural gaps resolved 2026-09-06 by portal evidence + this branch's implementation/decision).
...
**End of Release Gate Matrix.** State verified against `main` at `02a065070ba88c75f016956ae651a3269c55da63` plus this solution branch's additive changes.
