...
| **G2** Final ML candidate                | **PARTIAL**      | M17 locked in research (`origin/feat/shravan-final-ml-freeze`: checkpoint `experiments/m17-geonrw-struct-e01/checkpoints/best.pt`, SHA256 `D7C0BE91…EDAC`, Pearson 0.37 probe); NOT promoted to product (main still canonical DA-V2 Small) | Shravan        | **Yes**            |
...
| **G11** Installer                        | **PASS** (build) | NSIS 115,174,663 bytes (SHA256 `1310CA16…8383`), portable present; clean contents; `depthwiz_service.py` + `backend_bridge.py` via asarUnpack                                                                                              | Aryan          | **Yes** (physical) |
...
| **G19** GitHub/repository governance     | **PARTIAL**      | CI workflow created (`.github/workflows/ci.yml`); branch protection documented (`docs/github-branch-protection.md`, manual UI config required); `.github/CODEOWNERS` created this branch; PR #3 pending; no tags; API unverifiable (401)   | Shivam         | No                 |
