...
**Source of Truth:** Actual Git history + actual files on `main` at `24cce9825e66d789fe981063090c09a1c717c4e3` (`24cce98`)  
...
| G15  | SIH problem-statement compliance | All implemented PS capabilities in matrix verified; generalization beyond evidence not claimed |
...
| **G3** Real DA-V2 runtime                | **PASS**              | Real DA-V2 PyTorch inference verified in bridge integration tests & clean machine physical witness trial                                                                                                                                                                                                                                               | Shivam/Aryan   | No                 |
...
| **G9** Managed runtime                   | **PASS**              | `provision_runtime.py` (managed venv strategy, pip, git, checkpoint, idempotent); verified `ready:true`, `reused:true` on rerun                                                                                                                                                                                                                         | Shivam         | No                 |
...
| **G11** Installer                        | **PASS (SIGNED)**     | Signed NSIS Installer (`DepthWizard Setup 1.0.0.exe`, `115,579,824 bytes`), Authenticode signed (`CN=DepthWizard Release Candidate`, DigiCert RFC 3161 timestamp, SHA256 `2A974B51…`); portable present; clean contents                                                                                                                                 | Shivam/Aryan   | No                 |
...
| **G14** Physical Windows witness         | **PASS**              | Clean machine physical witness trial passed (verification items verified: install → launch → runtime → service → DA-V2 → calibration → DSM → mesh → renderer → flythrough → uninstall)                                                                                                                                                                 | Shivam         | No                 |
| **G15** SIH problem-statement compliance | **PASS**              | All implemented PS capabilities in the defined acceptance matrix were verified; scientific generalization/accuracy beyond the tested evidence is not claimed                                                                                                                                                                                         | Shivam         | No                 |
| **G15A** Solar-shadow sub-gate           | **EXCLUDED / NOT CLAIMED** | Solar-shadow height extraction investigated but explicitly excluded from shipped RC1 product scope (`docs/ps-solar-shadow-gap.md`)                                                                                                                                                                                                               | Shivam         | No                 |
...
| **G16** Scientific evidence              | **PASS (BASELINE)**   | GAMUS 32-tile pooled MAE 4.40 m / RMSE 5.86 m / R² 0.23 recorded honestly in `docs/SCIENTIFIC_EVIDENCE_PACKAGE.md`; GeoNRW M17 probe Pearson 0.37 (observed cross-city probe improvement; not a generalization proof); honest caveats documented | Shravan/Shivam | No                 |
...
| :--- | :--- | :--- |
| **Physical Windows Acceptance** | G3, G11, G14 | **RESOLVED**: Clean machine physical witness trial passed |
...
**End of Release Gate Matrix.** State verified against protected `main` at `24cce9825e66d789fe981063090c09a1c717c4e3` (`24cce98`).
