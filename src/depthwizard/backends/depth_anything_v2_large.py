...
Checkpoint: HF ``depth-anything/Depth-Anything-V2-Large`` file
...
path, ``DW_DAV2_LARGE_CKPT``, or ``checkpoints/depth_anything_v2_vitl.pth`` and
are never committed.
...
def _default_checkpoint_path() -> Path:
    """Resolve checkpoint: explicit env ``DW_DAV2_LARGE_CKPT`` else repo ``checkpoints/``."""
    env = os.environ.get("DW_DAV2_LARGE_CKPT")
    if env:
        return Path(env)
    try:
    ...
    def _require_torch(self) -> Any:
        try:
            ...
            raise ModelInferenceError(
                "torch is required for DepthAnythingV2LargeBackend but is not installed. "
    ...
    def estimate_depth(self, inspection: InputInspection) -> DepthResult:
        ...
        if not isinstance(inspection, InputInspection):
            raise InvalidInputError(
                f"DepthAnythingV2LargeBackend requires an InputInspection, "
        ...
        return DepthResult(
            ...
            provenance=ProductProvenance(
                ...
                generated_at=None,
                units=None,
                semantic_meaning="relative_depth from Depth Anything V2 Large (frozen inference)",
            ),
        )
