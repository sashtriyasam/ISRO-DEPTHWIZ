...
def evaluate_run(
    ...
    by_city_pooled: dict[str, dict[str, float]] | None = None,
) -> EvaluationRun:
    ...
    by_city_calibration: dict[str, dict[str, float]] = {}
    for city in cities:
        city_results = [result for result in results if result.city == city]
        count = len(city_results)
        by_city_calibration[city] = {
            "mean_scale": sum(result.calibration_scale for result in city_results) / count,
            "mean_offset": sum(result.calibration_offset for result in city_results) / count,
            "mean_residual_rmse": sum(result.calibration_rmse for result in city_results) / count,
            "control_pixels": float(
                sum(result.calibration_plan.control_pixels for result in city_results)
            ),
    ...
    return EvaluationRun(
        ...
        by_city=by_city,
        by_city_pooled=dict(by_city_pooled) if by_city_pooled else {},
        by_city_calibration=by_city_calibration,
