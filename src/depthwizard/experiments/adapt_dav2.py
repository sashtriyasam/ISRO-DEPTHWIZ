"""
M4 adaptation experiment runner: frozen DA-V2-Small + lightweight head (research).

Reproducible command:

    python -m depthwizard.experiments.adapt_dav2 \\
        --manifest manifests/gamus.m4.manifest.json \\
        --experiment-id dav2-gamus-head-m4-e02 \\
        --epochs 15 --lr 1e-3 --seed 0 \\
        --output experiments/dav2-gamus-head-m4-e02
...
def run_adaptation(
    ...
    m3_ref = {
        ...
        "different output semantics from M4 direct meters — NOT apples-to-apples.",
        "per_sample": m3_vals,
    ...
    results = {
        ...
        "kind": "adaptation-stage-A (frozen backbone + trainable head; research only)",
        "dataset": {
    ...
    print(f"M4 done: {experiment_id} best_val_mae={summary['best_value']:.4f} @epoch {summary['best_epoch']} -> {out}")
    return results
...
def main() -> None:
    ap = argparse.ArgumentParser(description="M4 frozen-backbone adaptation experiment")
