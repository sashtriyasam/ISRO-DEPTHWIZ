"""
M8 geographic training diversity experiment runner: frozen DA-V2-Small + lightweight head (research).
...
Train split: 8 DC + 8 PHL + 8 NYC (24 total) from train split.
...
def _select_train_recs(manifest_path: Path) -> list[GamusRecord]:
    """Select 8 DC + 8 PHL + 8 NYC samples from train split deterministically."""
...
def run_adaptation(
    ...
    # Select training samples: 8 DC + 8 PHL + 8 NYC from train split
    train_recs = _select_train_recs(Path("manifests/gamus.m4.manifest.json"))
...
def main() -> None:
    ap = argparse.ArgumentParser(description="M8 geographic training diversity experiment")
