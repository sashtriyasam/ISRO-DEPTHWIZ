...
- **Shravan milestone 8 scope**: geographic training diversity experiment on frozen M5 raw-meter model (`adapt_dav2_m8.py` runner, 8/8/8 city train composition), M5 config frozen, per-city/per-class/height-bin analysis vs M5. No training protocol change beyond city composition, no architecture change, no DA3, no calibration/DSM, no UI/Three.js.
