...
def _attach_significance(run, seed, resamples, confidence):
    """Append sample-level bootstrap comparisons (no pixels involved)."""
    import numpy as np

    from depthwizard.evaluation.significance import bootstrap_mean_interval, compare_groups

    per_sample = list(run.per_sample)
    cities = {}
    for result in per_sample:
        if result.city:
            cities.setdefault(result.city, []).append(result.metrics.mae)
    comparisons = []
    intervals = []
    names = sorted(cities)
    for name in names:
    ...
    for index in range(len(names)):
        for other in names[index + 1 :]:
            comparisons.append(
                compare_groups(
                    np.asarray(cities[names[index]]),
                    np.asarray(cities[other]),
                    names[index],
                    other,
                    "mae",
                    seed=seed,
...
def _aggregate_resume(args):
    ...
    if args.backend == "depth-anything-v2-small":
        try:
            from depthwizard.backends.depth_anything_v2 import CHECKPOINT_SHA256
    ...
    resume_dir = Path(args.aggregate_resume)
    samples = []
    ...
    records = load_resume_records(resume_dir, [sample.sample_id for sample in samples], identity)
    if not records:
        return _fail("EVALUATION_FAILED", "no identity-matching resume records found")
    results = []
    ...
    for sample in samples:
        ...
        if city:
            restore_accumulator(city_accumulators.setdefault(city, PooledAccumulator()), snapshot)
    if not results:
        return _fail("EVALUATION_FAILED", "no samples aggregated")
    run = evaluate_run(
        results,
        None,
        ...
        failures=failures,
        by_city_pooled={city: acc.summary() for city, acc in city_accumulators.items()},
        timing_seconds={"aggregation_only_seconds": 0.0},
        dataset_release=document.get("release"),
        manifest_checksum=manifest_checksum,
        device=None,
    )
    if args.bootstrap_ci:
        run = _attach_significance(
            run, seed=args.seed, resamples=args.resamples, confidence=args.confidence
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--aggregate-resume", default=None)
    ...
    if args.aggregate_resume:
        return _aggregate_resume(args)
    ...
    if args.report:
        ...
        if significance:
            for comparison in significance.get("comparisons", []):
                lines.append(
                    f"{comparison['group_a']} vs {comparison['group_b']} "
                    f"({comparison['metric']}): delta {comparison['observed_delta']:.4f} "
