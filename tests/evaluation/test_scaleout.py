...
def test_aggregate_resume_rebuilds_exact_run(tmp_path) -> None:
    """Aggregate-only CLI path rebuilds pooled metrics with no inference."""
    import json
    import subprocess
    import sys
    from pathlib import Path

    from depthwizard.evaluation.resume import run_identity, save_record
    from depthwizard.evaluation.smoke import run_smoke
    ...
    repo = _subprocess.run(["git", "rev-parse", "HEAD"], capture_output=True, text=True, timeout=30)
    repository_sha = repo.stdout.strip() if repo.returncode == 0 else None
    identity = run_identity(
        manifest_checksum,
        "smoke-backend",
        None,
    ...
    save_record(
        ...
        result,
        identity,
        _snapshot(calibrated, reference),
    )
...
def _snapshot(calibrated, reference):
    """Snapshot helper mirroring the CLI fold."""
    import numpy as np

    errors = np.asarray(calibrated - reference, dtype=np.float64).ravel()
    references = np.asarray(reference, dtype=np.float64).ravel()
